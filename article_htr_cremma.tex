...
\begin{itemize}
  \item \textbf{CREMMA-Medieval} (8 documents français, XIIIe) : dépôt HTR-United officiel
...
Lignes texte courant (filtré) & 23~575 \\
Langue dominante & AF \\
...
\fontsize{10}{11}\selectfont
\centering
\caption{Performances du meilleur modèle (Run 4)}
\begin{tabular}{L{1.5cm}C{3cm}C{1.5cm}}
\toprule
...
Tous les modèles fine-tunés sont publiés sur HuggingFace \cite{huggingface2020} (\texttt{legb/htr-cremma-medieval}) sous licence CC-BY 4.0 :
...
Le dépôt GitHub (github.com/Loulou441/htr-manuscrits-XIIIe-siecle) contient :
...
Les artefacts (modèles safetensors, scripts pré-traitement, notebooks d'entraînement) sont publiquement disponibles pour reproduit et étendy ce travail. La transparence méthodologique et la documentation complète des runs expérimentales constituent le principal apport reproduisibilité de ce projet, dans la continuité des pratiques d'ouverture de la communauté HTR \cite{crowdsourcing_htr2020}.
...
Les auteurs remercient Gallica (BnF) \cite{gallica2023} pour l'accès aux images numériques et le projet CREMMA \cite{cremma2022} (HTR-United) \cite{htr-united2022} pour les modèles pré-entraînés et ressources de corpus. Les expériences ont bénéficié des GPUs cloud de Kaggle et Google Colab.
...
BnF fr. 25516 & AF & XIIIe & \textit{Gothic Textualis} & 717 & CREMMA-Medieval \\
...
\midrule
\textbf{TOTAL (brut avant équilibrage)} & & & & \textbf{23575} & \textbf{4 corpus} \\ 
\bottomrule
\end{tabular}
\end{table}
